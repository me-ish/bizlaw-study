...
export default function ExamHomePage() {
  ...
  useEffect(() => {
    ...
    if (qp.answered > 0) setAccuracy(Math.round((qp.correct / qp.answered) * 100));
    setTopicStats(qp.topicStats ?? {});
  }, [examId]);

  function handleReset() {
    if (!confirm("学習進捗をリセットしますか？")) return;
    resetQuizProgress(examId);
    resetFlashcardProgress(examId);
    resetNotesProgress(examId);
    setQuizPct(0);
    setCardPct(0);
    setAccuracy(null);
    setTopicStats({});
  ...
  const studySteps = [
    { step: 1, icon: "📖", label: "まとめノート", href: `/${examId}/notes`, desc: "各分野の全体像と重要論点を把握" },
    { step: 2, icon: "🃏", label: "単語帳", href: `/${examId}/flashcards`, desc: "重要用語を暗記・定着" },
    { step: 3, icon: "📝", label: "模擬テスト", href: `/${examId}/quiz`, desc: "知識を問題で確認・弱点発見" },
    { step: 4, icon: "⚡", label: "直前チェック", href: `/${examId}/review`, desc: "試験直前に要点を一気に復習" },
  ];
